...
def get_language_families():
    ...
    return {
        'Romance': ['it', 'es', 'fr', 'pt', 'ro'],  # Added Romanian
        'Germanic': ['de', 'af', 'nl', 'sv', 'da', 'no', 'en'],
        'Slavic': ['ru', 'pl'],
        'Sino-Tibetan': ['zh', 'bo'],
        'Other': ['mt', 'ja', 'ar', 'ko', 'hi', 'tr', 'vi', 'th', 'el']  # Added Vietnamese, Thai, Greek
    }
...
def visualize_results_as_confusion_matrix(folder):
    ...
    for file in files:
        with open(os.path.join(folder, file), "r", encoding="utf-8") as f:
            ...
            # Get baseline and enhanced scores
            baseline = data["baseline"]["f1_score"]
            enhanced = data["enhanced"]["f1_score"]
            improvement_raw = data["f1_improvement"]
    ...
    sns.heatmap(matrix, xticklabels=activate_labels, yticklabels=test_labels,
                ...
                center=0, cbar_kws={'label': 'F1 Improvement (%)'})
    ...
    # plt.title("F1 Score Improvement by Test and Activated Language\n(Grouped by Language Family)")
    plt.xlabel("Activated Language", fontsize=20)
    plt.ylabel("Test Language", fontsize=20)
    plt.savefig(f"{args.output_path}/xquad_family_grouped.png", dpi=300, bbox_inches='tight')
    plt.tight_layout()
    plt.show()
