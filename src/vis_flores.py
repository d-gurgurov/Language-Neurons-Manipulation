...
def visualize_results_as_confusion_matrix(folder):
    ...
    for file in files:
        with open(os.path.join(folder, file), "r", encoding="utf-8") as f:
            ...
            baseline = data["baseline"]["bleu_score"]
            enhanced = data["enhanced"]["bleu_score"]
            improvement_raw = data["improvement"]
            if isinstance(improvement_raw, dict):
                improvement = improvement_raw["bleu_score"]
            else:
    ...
    plt.savefig(f"{args.output_path}/flores_family_grouped.png", dpi=300, bbox_inches='tight')
    plt.tight_layout()
    plt.show()
