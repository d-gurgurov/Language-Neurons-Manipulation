...
def get_language_families():
    ...
    return {
        ...
        'Slavic': ['ru', 'pl', 'bg'], 
        'Sino-Tibetan': ['zh', 'bo'],
        'Other': ['mt', 'ja', 'ar', 'ko', 'hi', 'tr', 'el', 'sw', 'th', 'ur', 'vi']  # Added Greek, Swahili, Thai, Urdu, Vietnamese
    }
...
def visualize_results_as_confusion_matrix(folder):
    ...
    for file in files:
        with open(os.path.join(folder, file), "r", encoding="utf-8") as f:
            ...
            improvement_raw = data["improvements"]["accuracy"]

            test_langs.add(test)
            activated_langs.add(activate)
            results_dict[(test, activate)] = improvement_raw

    print(f"Sample results_dict entries: {list(results_dict.items())[:5]}")
    print(f"Test languages found: {sorted(test_langs)}")
    print(f"Activated languages found: {sorted(activated_langs)}")

    # Since the keys in results_dict are already in 2-letter format,
    # we don't need to convert them - they're already what we want
    test_langs_old = list(test_langs)
    activated_langs_old = list(activated_langs)
    ...
    print(f"Test languages ordered: {test_langs_ordered}")
    print(f"Activated languages ordered: {activated_langs_ordered}")

    # Create matrix using the 2-letter codes directly
    matrix = np.zeros((len(test_langs_ordered), len(activated_langs_ordered)))
    annot_matrix = np.empty((len(test_langs_ordered), len(activated_langs_ordered)), dtype=object)

    for i, test in enumerate(test_langs_ordered):
        for j, activate in enumerate(activated_langs_ordered):
    ...
    sns.heatmap(matrix, xticklabels=activate_labels, yticklabels=test_labels,
                ...
                center=0, cbar_kws={'label': 'Accuracy Improvement (%)'})  # Changed from BLEU to Accuracy
    ...
    # plt.title("Accuracy Improvement by Test and Activated Language\n(Grouped by Language Family)")
    plt.xlabel("Activated Language", fontsize=20)
    plt.ylabel("Test Language", fontsize=20)
    plt.savefig(f"{args.output_path}/xnli_family_grouped.png", dpi=300, bbox_inches='tight')
    plt.tight_layout()
    plt.show()
